...
Two-way anova with block and nitro nested within block as random effects. This is a case where glmmTMB() errors out for some reason but lmer can handle the model. (not sure why)
```{r}
mm2 <- lme4::lmer(yield ~ gen*nitro+(1|loc/nitro), data=gomez_summarized)
Anova(mm2)
```
...
```{r}
mm3 <- lme4::lmer(yield ~ gen*nitro+(1|loc/nitro/gen), data=gomez)
